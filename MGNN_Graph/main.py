...
class Net(torch.nn.Module):
    def __init__(self, in_channel, hidden_dims, out_channel, args=None):
        ...
        self.lin1 = Linear(dim, dim_out)
        self.lin2 = Linear(dim_out, out_channel)

    def forward(self, graphs_data):
        graph_org = graphs_data[0]
        graph_org.edge_weight4norm = torch.tensor([1.0] * graph_org.edge_index.shape[1]).to(device)
        ...
        h_emb = self.lin1(h).relu()
        
        h = F.dropout(h_emb, p=0.5, training=self.training)
        h = self.lin2(h)
        return F.log_softmax(h, dim=1),h_emb
...
if __name__ == '__main__':
    cross_validation_with_val_set('FRANKENSTEIN', folds=5, weight_decay=0, gen_x='max_degree')  
