...
class MotifGraph(object):
    def __init__(self, target_motif, dataset_name, pad=0, cache_dir='', force_directed=False):
    ...
    def __call__(self, data):
        if self.pad > data.x.shape[1]:
            data.x = torch.cat([data.x,torch.zeros((data.x.shape[0],self.pad-data.x.shape[1]))],dim=-1)
        N = data.num_nodes
